...
- ✅ **스크래치 소비자 시나리오** — 별도 디렉토리에서 인증 정보 없이 install/extends 해석 모두 성공.
   ...
   ```
- 📝 **주의사항** — `@minjun0219/biome-config` 는 기본으로 `vcs.useIgnoreFile: true` 를 켜기 때문에 소비자 프로젝트가 Git 저장소가 아니거나 `.gitignore` 가 없으면 `biome check .` 이 실패합니다. 실제 소비자 프로젝트에는 대부분 이미 존재하므로 문제되지 않지만 이 사실은 소비자 README 에 명시하는 편이 나을 수 있음.
