...
이 설정은 `vcs.useIgnoreFile: true` 를 켜므로 소비자 프로젝트에 `.gitignore` 가 존재해야 `biome check .` 이 정상 실행됩니다. git 저장소가 아닌 순수 실험 디렉토리에서는 최소한 빈 `.gitignore` 라도 두거나 소비자 `biome.jsonc` 에서 `vcs.enabled: false` 로 재정의하세요.
