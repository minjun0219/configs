...
이 설정은 `vcs.useIgnoreFile: true` 를 켜므로 소비자 프로젝트에 `.gitignore` 가 존재해야 `biome check .` 이 정상 실행됩니다. Git 저장소가 아닌 순수 실험 디렉토리에서는 `git init` 을 실행하여 Git 저장소로 초기화하거나, 소비자 `biome.jsonc` 에서 `vcs.enabled: false` 로 재정의하세요.
